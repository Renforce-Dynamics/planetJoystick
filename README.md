...
仓库内包含独立发布的 `planetj-protocol`，发送端和消费端共享编解码。既不依赖 Planet-PingPong，也不依赖 Cadence 执行内核、SDK、模型或 NumPy。A3 按键 profile 在 `cadence-rally` 的应用 bundle 中，测试目录里的旧 profile 仅用作兼容性 fixture。
